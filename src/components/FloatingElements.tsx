    ...
    <div className="fixed inset-0 pointer-events-none z-0">
      ...
      {[...Array(12)].map((_, i) => {
        ...
        return (
          <motion.div
            key={`tech-${i}`}
            className="absolute w-8 h-8 text-blue-500/30"
            style={{
              left: `${10 + (i * 8)}%`,
              top: `${20 + (i * 6)}%`,
            }}
      ...
      {['React', 'TypeScript', 'Node.js', 'Python', 'MongoDB', 'Express'].map((tech, i) => (
        <motion.div
          key={`code-${i}`}
          className="absolute text-blue-400/40 font-mono text-sm font-bold bg-blue-900/10 px-3 py-1 rounded-lg border border-blue-500/20"
          style={{
            right: `${5 + i * 15}%`,
            top: `${40 + i * 10}%`,
          }}
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div
          key={`shape-${i}`}
          className={`absolute w-6 h-6 ${i % 3 === 0 ? 'bg-blue-500/30' : i % 3 === 1 ? 'bg-blue-600/30' : 'bg-blue-400/30'} ${i % 2 === 0 ? 'rounded-full' : 'rounded-lg rotate-45'}`}
          style={{
